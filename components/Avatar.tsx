"use client";
export const dynamic = "force-dynamic"; // ensures no static pre-render crash
...
const STAGES_CONFIG: Record<PetStage, { name: PetStage; imgSrc: string }> = {
  ...
  "Young Cub": {
    name: "Young Cub",
    imgSrc:
      "https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2027,%202025,%2011_49_04%20PM.png?updatedAt=1756315173024",
  },
  "Teen Lion": {
    name: "Teen Lion",
    imgSrc:
      "https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2027,%202025,%2002_29_46%20PM.png?updatedAt=1756314700059",
  },
  "Adult Lion": {
    name: "Adult Lion",
    imgSrc:
      "https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2027,%202025,%2002_29_44%20PM.png?updatedAt=1756314701116",
  },
  "Legendary Lion": {
    name: "Legendary Lion",
    imgSrc:
      "https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2027,%202025,%2002_29_37%20PM.png?updatedAt=1756314699490",
  },
};
...
interface AvatarCustomizationModalProps {
  ...
  setEquippedItems: React.Dispatch<
    React.SetStateAction<Record<EquipTab, string | null>>
  >;
}
...
const AvatarCustomizationModal: React.FC<AvatarCustomizationModalProps> = ({
  ...
  const INVENTORY: Record<EquipTab, Item[]> = {
    Outfits: [
      {
        ...
        label: "Elderwood",
        imgSrc:
          "https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2029,%202025,%2011_49_34%20AM.png?updatedAt=1756446094274",
      },
    ],
    Bodywear: [
      {
    ...
    Mane: [
      {
        id: "ornaments",
        label: "Gilded Mane Ornaments",
        imgSrc:
          "https://ik.imagekit.io/erriqyxye/Ipay%20RFP/ChatGPT%20Image%20Aug%2026,%202025,%2004_09_30%20AM.png?updatedAt=1756157985240",
      },
    ],
  };
  const COLOR_OPTIONS: Item[] = [
    { id: "golden", label: "Golden", emoji: "🌟" },
    { id: "charcoal", label: "Charcoal", emoji: "⚫" },
    { id: "ivory", label: "Ivory", emoji: "⚪" },
  ];
  const POSE_OPTIONS: Item[] = [
    { id: "regal", label: "Regal", emoji: "👑" },
    { id: "pounce", label: "Pounce", emoji: "🐾" },
    { id: "resting", label: "Resting", emoji: "😴" },
  ];
  const BG_OPTIONS: Item[] = [
    { id: "savannah", label: "Savannah Sunrise", emoji: "🌅" },
    { id: "watering-hole", label: "Watering Hole", emoji: "💧" },
    { id: "canopy", label: "Jungle Canopy", emoji: "🌳" },
  ];

  const [panel, setPanel] = React.useState<"Equipment" | "Customization">(
    "Equipment"
  );
  ...
  function pickCustomize(id: string) {
    if (customTab === "Fur Color") setColorId(id);
    if (customTab === "Pose") setPoseId(id);
    if (customTab === "Environment") setBgId(id);
  }
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full bg-surface rounded-t-2xl p-4 border-t border-divider max-h-[70%] flex flex-col"
      >
        ...
        <div className="overflow-y-auto no-scrollbar">
          ...
          {panel === "Equipment" ? (
            <section>
              <TabBar
                tabs={["Outfits", "Bodywear", "Paws", "Mane"]}
                active={equipTab}
                onChange={(t) => setEquipTab(t as EquipTab)}
              />
              <div className="mt-4 grid grid-cols-3 gap-3">
                {activeInventory.map((it) => (
                  <TileButton
                    key={it.id}
                    label={it.label}
                    imgSrc={it.imgSrc ?? "placeholder.png"}
                    active={equippedItems[equipTab] === it.id}
            ...
            <section>
              <TabBar
                tabs={["Fur Color", "Pose", "Environment"]}
                active={customTab}
                onChange={(t) => setCustomTab(t as CustomTab)}
              />
              <div className="mt-4 grid grid-cols-3 gap-3">
                {activeCustomize.map((it) => (
                  <TileButton
                    key={it.id}
                    label={it.label}
                    emoji={it.emoji}
                    active={
                      (customTab === "Fur Color" && colorId === it.id) ||
                      (customTab === "Pose" && poseId === it.id) ||
                      (customTab === "Environment" && bgId === it.id)
                    }
                    onClick={() => pickCustomize(it.id)}
                  />
                ))}
              </div>
            </section>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <img
          src="https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2027,%202025,%2011_59_32%20PM.png?updatedAt=1756319565919"
          alt="Sad Pet"
        ...
        <p className="text-text-secondary mb-6">
          Complete a recovery mission or spend diamonds to regain happiness.
        </p>
        <div className="space-y-3">
          ...
          <button
            onClick={onSpendDiamonds}
            className="w-full bg-secondary-action hover:bg-secondary-action-dark text-text-inverse font-bold py-3 rounded-xl transition-colors flex items-center justify-center space-x-2"
          >
...
const WellbeingMeter: React.FC<{
  ...
  return (
    <div className="flex flex-col items-center space-y-2">
      <div
        className={`relative w-20 h-20 rounded-full flex items-center justify-center text-text-primary font-bold text-lg`}
      >
...
const HappinessMissionCard: React.FC<{
  icon: React.ReactNode;
  title: string;
  description: string;
  onClick: () => void;
  ...
  <div className="bg-background p-3 rounded-lg flex items-center space-x-3">
    ...
    <button
      onClick={onClick}
      className="bg-primary text-text-inverse font-semibold text-sm px-4 py-1.5 rounded-md hover:bg-primary-hover transition-colors"
    >
      Go
    </button>
  </div>
...
const Avatar: React.FC<AvatarProps> = ({
  ...
  const handleDecayClick = (level: number) => {
    ...
    switch (level) {
      ...
      case 1:
        setHappiness(100);
        setDecayImage(
          "https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2027,%202025,%2011_53_18%20PM.png?updatedAt=1756319565628"
        );
        break;
      case 2:
        setHappiness(50);
        setDecayImage(
          "https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2027,%202025,%2011_59_32%20PM.png?updatedAt=1756319565919"
        );
        break;
      case 3:
        setHappiness(40);
      ...
      case 4:
        setHappiness(30);
        setDecayImage(
          "https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2028,%202025,%2012_23_15%20AM.png?updatedAt=1756319552906"
        );
        break;
      default:
        break;
  ...
  const elderwoodEquippedImage =
    "https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2029,%202025,%2012_20_12%20PM.png?updatedAt=1756446626508";
  const baseImage = decayImage || STAGES_CONFIG[previewStage].imgSrc;
  const imageToDisplay =
    equippedItems.Outfits === "crown" ? elderwoodEquippedImage : baseImage;
    ...
    <div className="p-4 text-text-primary flex flex-col gap-4 relative">
      <AnimatePresence>
        {showRecoveryModal && (
          <RecoveryModal
            ...
            onViewMission={() => handleRecovery("mission")}
            onSpendDiamonds={() => handleRecovery("spend")}
          />
        )}
      </AnimatePresence>
      {showInfo && (
        <PageInfoModal title="My Pet" onClose={() => setShowInfo(false)}>
          <p className="font-bold">
            Behold, your loyal companion and the heart of your adventure!
            Tending to your Pet is the key to unlocking true power.
          </p>
          <ul className="list-disc list-inside space-y-2 mt-2">
            <li>
              <strong>Evolution & Level:</strong> The XP bar is the story of
              your Pet's growth. Every mission and game fuels its evolution,
              transforming it into new, magnificent forms and unlocking
              uncharted islands.
            </li>
            <li>
              <strong>Happiness Meter:</strong> A joyful Pet is a generous one!
              Keep its happiness high, and it will often surprise you with gifts
              of Coins and XP. But if its spirits fall, your journey will stall!
            </li>
            <li>
              <strong>Happiness Missions:</strong> Restore your Pet's zest for
              adventure! These quick tasks are the perfect way to boost
              happiness.
            </li>
            <li>
              <strong>Customize:</strong> Tap the palette icon to enter the
              wardrobe. Style your Pet with unique gear to craft a look that is
              truly legendary.
            </li>
          </ul>
        </PageInfoModal>
      ...
      <div className="flex justify-center items-center relative">
        ...
        <button
          ...
          className="absolute right-0 w-8 h-8 rounded-full bg-gray-300 flex items-center justify-center hover:bg-gray-400 transition-colors flex-shrink-0"
        >
      ...
      <div>
        <div className="flex justify-between items-center mb-1">
          ...
          <span className="text-sm font-semibold text-text-secondary">
            {xpProgressInLevel} / {totalXpForLevel} XP
          </span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            className="bg-primary h-2 rounded-full transition-all duration-500 ease-out"
            style={{ width: `${(xpProgressInLevel / totalXpForLevel) * 100}%` }}
          ></div>
        </div>
      </div>
      ...
      <div className="flex flex-col">
        <section
          ...
          onClick={() => {
            if (happiness === 50 && decayImage) setShowRecoveryModal(true);
          }}
        >
          <div className="relative flex flex-col items-center">
            ...
            />
            <div className="bg-black/40 text-white font-bold text-lg px-4 py-1 rounded-full -mt-4 backdrop-blur-sm">
              {decayImage ? "" : STAGES_CONFIG[previewStage].name}
            </div>
            <button
              className="absolute top-4 left-4 p-2.5 bg-black/50 rounded-full hover:bg-black/70 transition-colors backdrop-blur-sm border border-white/20"
            ...
            <button
              ...
              className="absolute top-4 right-4 p-2.5 bg-black/50 rounded-full hover:bg-black/70 transition-colors backdrop-blur-sm border border-white/20"
      ...
      <section className="bg-surface p-4 rounded-2xl shadow-md border border-divider">
        <div className="flex justify-center">
          <WellbeingMeter
            label="Happiness"
            value={happiness}
            icon={<StarIcon className="w-10 h-10 text-warning" />}
        ...
        {happiness <= 30 && (
          <div className="text-center text-sm text-error font-semibold flex items-center justify-center gap-1 bg-red-100 p-2 rounded-lg border border-error/30 mt-4">
            <InfoIcon className="w-5 h-5 flex-shrink-0" />
            <span>
              You have stop growing, raise happiness to continue earning XP for
              pet and get rewards.
            </span>
          </div>
        )}
      ...
      <section className="bg-surface p-4 rounded-2xl shadow-md border border-divider">
        <div className="flex flex-wrap gap-2 justify-center">
          {["0", "1", "2", "3", "4", "5", "G1", "G2", "G3", "G4", "G5"].map(
            (label) => {
      ...
      <section className="bg-surface p-4 rounded-2xl shadow-md border border-divider">
        ...
        <div className="space-y-3">
          <HappinessMissionCard
            icon={<MissionIcon className="w-6 h-6" />}
            title="Complete a Mission"
            description="Makes your pet feel accomplished."
            onClick={() => setActivePage("championship")}
          />
          <HappinessMissionCard
            icon={<PlayActionIcon className="w-6 h-6" />}
            title="Play a Game"
            description="Playtime is the best time!"
            onClick={() => setActivePage("championship")}
          />
          <HappinessMissionCard
            icon={<ShopIcon className="w-6 h-6" />}
            title="Visit the Shop"
            description="A new toy always brings joy."
    ...
    {tabs.map((t) => (
      <button
        ...
        className={`px-3 py-2 rounded-xl text-sm font-semibold ${
          active === t
            ? "bg-primary-light text-secondary-action"
    ...
    className={`aspect-square rounded-2xl flex flex-col items-center justify-center p-1 text-center
      ${
        active
          ...
          : "bg-gray-100 ring-1 ring-divider"
      }`}
  >
    ...
    <span className="text-[11px] leading-tight mt-1 opacity-90 shrink-0 block">
      {label}
    </span>
  </button>
);
