...
const RewardModal: React.FC<{ score: number; reward: number; onClaim: () => void; }> = ({ score, reward, onClaim }) => {
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            ...
            >
                ...
                <button onClick={onClaim} className="w-full bg-primary text-text-inverse font-bold py-3 rounded-xl text-lg transform hover:scale-105 transition-transform">Claim & Close</button>
            </motion.div>
        </div>
    );
};

export const DailySpinGame: React.FC<DailySpinGameProps> = ({ setCoins, onBack }) => {
    ...
    return (
        <div 
            ...
            style={{ backgroundImage: "url('https://ik.imagekit.io/erriqyxye/Bkash/ChatGPT%20Image%20Aug%2027,%202025,%2010_45_57%20AM%20(1).png?updatedAt=1756268262166')" }}
            onClick={!showRewardModal ? handleGameClick : undefined}
            role="button"
            aria-label="Click to get your reward"
        >
            {!showRewardModal && onBack && <BackButton onClick={onBack} variant="light" />}
