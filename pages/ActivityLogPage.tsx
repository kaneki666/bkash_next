...
const ActivityLogPage: React.FC<PageProps> = ({ setActivePage, onBack }) => {
  const fullActivityLog = [
    ...
    {
      icon: <CoinIcon className="w-6 h-6 text-warning-dark" />,
      text: "Spent 100 Coins to enter Jungle Race",
      time: "3 hours ago",
    },
    {
      icon: <SkinsIcon className="w-6 h-6 text-error" />,
      text: "Purchased 'Cyber Visor' skin",
      time: "1 day ago",
    },
    {
      icon: <XPIcon className="w-6 h-6 text-warning" />,
      text: "Earned 100 XP from Daily Login",
      time: "1 day ago",
    },
    {
      icon: <CoinIcon className="w-6 h-6 text-primary" />,
      text: "Earned 500 Coins from Daily Login",
      time: "1 day ago",
    },
    {
      icon: <MissionIcon className="w-6 h-6 text-primary" />,
      text: "Completed 'Top-up any amount'",
      time: "2 days ago",
    },
    {
      icon: <PodiumIcon className="w-6 h-6 text-text-secondary" />,
      text: "Placed #24 in Vine Puzzle",
      time: "2 days ago",
    },
  ];
  ...
  return (
    <div className="p-4 text-text-primary relative h-full flex flex-col">
      ...
      <div className="space-y-3 flex-grow overflow-y-auto no-scrollbar pr-2">
        {fullActivityLog.map((activity, index) => (
