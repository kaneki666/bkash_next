...
const initialMissionsData: Mission[] = [
  ...
  {
    id: 1,
    title: "Top-up any amount",
    reward: "+50 XP & Happiness",
    xp: 50,
    progress: 0,
    isCompleted: false,
    category: "starter",
  ...
  {
    id: 10,
    title: "Pay an Internet Bill",
    reward: "+30 XP & Happiness",
    xp: 30,
    progress: 0,
    isCompleted: false,
    category: "starter",
  ...
  {
    id: 11,
    title: "Pay a Postpaid Mobile Bill",
    reward: "+40 XP & Happiness",
    xp: 40,
    progress: 0,
    isCompleted: false,
    category: "starter",
  ...
  {
    id: 2,
    title: "Play 1 Tournament",
    reward: "+25 XP & Happiness",
    xp: 25,
    progress: 0,
    isCompleted: false,
    category: "starter",
  ...
  {
    id: 6,
    title: "Make a QR Payment of BDT 100",
    reward: "+75 XP",
    xp: 75,
    progress: 0,
    isCompleted: false,
  ...
  {
    id: 7,
    title: "Reach Top 10 in a Leaderboard",
  ...
  {
    id: 9,
    title: "Win a Global Tournament",
    reward: "+1000 XP, +1000 Coins",
    xp: 1000,
    progress: 0,
    isCompleted: false,
    category: "legend",
    ...
    return (
      <div className="bg-surface p-4 rounded-lg shadow-md opacity-60 cursor-not-allowed">
        <div className="flex justify-between items-center">
          ...
          <button
            disabled
            className="bg-disabled-fill text-text-disabled font-bold px-4 py-1 rounded-md text-sm"
          >
            Locked
          </button>
        </div>
...
const FreePlayModal: React.FC<{ onPlay: () => void; onClose: () => void }> = ({
  onPlay,
  onClose,
}) => (
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 animate-fade-in">
    ...
    >
      ...
      <h2 className="text-2xl font-extrabold text-primary mb-2">
        You've Earned a Free Play!
      </h2>
      <p className="text-text-secondary mb-6">
        Your mission streak has paid off. Time for a game!
      </p>
...
const QuestHub: React.FC<QuestHubProps> = ({
  onMissionComplete,
  setActivePage,
  onBack,
  activeTab,
  ...
  const [activeQuestCategory, setActiveQuestCategory] =
    useState<MissionCategory>(forcedCategory || "starter");

  const questCategories: MissionCategory[] = [
    "starter",
    "rookie",
    "conqueror",
    "legend",
  ...
  const handleCompleteMission = (missionId: number) => {
    ...
    if (
      !missionToComplete ||
      missionToComplete.isCompleted ||
      missionToComplete.locked
    )
      return;

    addNotification(
      `Mission Complete: ${missionToComplete.title}`,
      "mission",
      "quest"
    );
  ...
  const filteredMissions = missions.filter(
    (m) => m.category === activeQuestCategory
  );
  ...
  return (
    <div className="p-4 text-text-primary relative">
      ...
      {showInfo && (
        <PageInfoModal
          title={achievementsOnly ? "Achievements" : "Quest & Achievement Hub"}
          onClose={() => setShowInfo(false)}
        >
          {achievementsOnly ? (
            <>
              <p>
                This is your hall of legendary accomplishments. Each achievement
                represents a major milestone in your journey.
              </p>
              <p className="mt-2">
                Completing these long-term goals will test your dedication and
                skill, rewarding you with exclusive badges to display on your
                Trophy Rack. Fulfill your destiny and collect them all!
              </p>
            </>
          ) : (
            <>
              <p>
                This is your command center for action and glory! Here you'll
                find opportunities to prove your skills and earn amazing
                rewards.
              </p>
              <ul className="list-disc list-inside space-y-2 mt-2">
                <li>
                  <strong>Missions:</strong> Think of these as your daily and
                  weekly challenges. They are the fastest way to earn XP to
                  evolve your Pet, stock up on Coins for the shop, and build
                  your Mission Streak for a free play in the daily{" "}
                  <strong>Flip & Match</strong> game!
                </li>
                <li>
                  <strong>Achievements:</strong> These are your legendary
                  milestones. Completing them signifies true mastery and earns
                  you prestigious badges for your Trophy Rack. Show them off
                  with pride!
                </li>
              </ul>
            </>
      ...
      <h1 className="text-3xl font-extrabold text-center mb-6">
        {achievementsOnly ? "Achievements" : "Quest Hub"}
      </h1>
        ...
        <div>
          <div className="bg-surface p-4 rounded-lg mb-6 border border-primary-light shadow-md">
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-xl font-bold text-secondary-action">
                Mission Streak
              </h3>
              <span className="font-bold text-secondary-action-dark bg-primary-light px-3 py-1 rounded-full text-sm">
                {missionStreak} / 10
              </span>
            </div>
                ...
                {[...Array(10)].map((_, i) => (
                  <div
                    ...
                    className={`h-3 flex-1 rounded-full transition-colors duration-300 ${
                      i < missionStreak ? "bg-primary" : "bg-gray-200"
                    }`}
                  ></div>
                ))}
              </div>
            ...
            <p className="text-center text-sm text-text-secondary mt-4">
              Complete missions to earn free treasure spins!
            </p>
          </div>
            ...
            <div className="w-full bg-gray-200 rounded-xl p-1 flex mb-6">
              {questCategories.map((cat) => (
                <button
                  ...
                  className={`w-1/4 py-2 text-sm rounded-lg font-semibold transition-colors ${
                    activeQuestCategory === cat
                      ? "bg-primary text-text-inverse"
                      : "text-text-secondary"
                  }`}
                >
          ...
          <div className="space-y-4">
            {filteredMissions.length > 0 ? (
              ...
              <p className="text-center text-text-secondary py-8">
                No missions in this category right now. Check back later!
              </p>
            )}
          </div>
        </div>
