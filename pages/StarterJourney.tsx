...
import { PageProps, MilestoneData, Notification } from '../types';
...
export const StarterJourney: React.FC<StarterJourneyProps> = (props) => {
        ...
        <div className="relative h-full flex flex-col bg-background">
            <div className="p-4 bg-background flex-shrink-0">
                <div className="relative flex items-center justify-center">
                    ...
                    <h1 className="text-2xl font-extrabold text-center">Starter Journey</h1>
                </div>
                <div className="w-full bg-gray-200 rounded-xl p-1.5 flex mt-4">
                    <button onClick={() => setActiveTab('journey')} className={`w-1/2 py-2 rounded-lg font-semibold transition-colors ${activeTab === 'journey' ? 'bg-primary text-text-inverse' : 'text-text-secondary'}`}>Journey</button>
                    <button onClick={() => setActiveTab('quests')} className={`w-1/2 py-2 rounded-lg font-semibold transition-colors ${activeTab === 'quests' ? 'bg-primary text-text-inverse' : 'text-text-secondary'}`}>Quests</button>
                </div>
            </div>
            ...
            <div className="flex-grow overflow-y-auto no-scrollbar">
                {activeTab === 'journey' && (
                    <div 
                        ...
                        style={{backgroundImage: "url('https://ik.imagekit.io/erriqyxye/Ipay%20RFP/390%20x%20800.jpg?updatedAt=1756152557533')"}}
                    >
                        <RewardMap 
