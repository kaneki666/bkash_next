...
import { CheckIcon, InfoIcon, UsersIcon } from "../components/icons";
...
const missions: Mission[] = [
  {
    id: 1,
    title: "Community Payment Spree",
    description: "Make 10,000 QR payments as a community.",
    currentProgress: 7854,
    target: 10000,
    reward: "+500 Coins for all participants",
    participants: 1245,
    isJoined: true,
  },
  {
    id: 2,
    title: "Top-Up Titans",
    description: "Cumulatively top-up BDT 50,000 as a community.",
    currentProgress: 23150,
    target: 50000,
    reward: "+1 Free Treasure Spin",
    participants: 876,
    isJoined: false,
  },
  {
    id: 3,
    title: "Friend Fiesta",
    description: "Refer 500 new users together.",
    currentProgress: 450,
    target: 500,
    reward: 'Exclusive "Socialite" Badge',
    participants: 350,
    isJoined: false,
  },
];
  ...
  return (
    ...
    >
      ...
      <div className="w-full bg-gray-200 rounded-full h-4 relative">
        ...
        />
        <span className="absolute inset-0 text-center text-xs font-bold text-white flex items-center justify-center">
          {Math.round(progressPercent)}%
        </span>
      </div>
      <p className="text-xs text-right text-text-secondary mt-1">
        {mission.currentProgress.toLocaleString()} /{" "}
...
const TeamUpChallenge: React.FC<PageProps> = ({ onBack }) => {
    ...
    <div className="p-4 text-text-primary h-full flex flex-col">
      <AnimatePresence>
        {showInfo && (
          ...
          >
            <p>
              Strength in numbers! The Team Up Challenge calls for all champions
              to unite against colossal goals that no single hero could conquer
              alone.
            </p>
            <p className="mt-2">
              Every action you take contributes to a shared community objective.
              If the goal is met before time expires, *every* participant reaps
              the glorious rewards. Join the collective and let's achieve
              victory, together!
            </p>
          </PageInfoModal>
        )}
      ...
      <div className="relative flex items-center justify-center flex-shrink-0 mb-4">
        ...
        <h1 className="text-3xl font-extrabold text-center px-10">
          Team Up Challenge
        </h1>
        <button
      ...
      <p className="text-center text-text-secondary mb-6">
        Join forces with the community to complete missions and earn exclusive
        rewards together!
      </p>
...
export default TeamUpChallenge;
