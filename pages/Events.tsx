...
const EventBanner: React.FC<{
  ...
  return (
    <motion.div
      ...
      style={{
        boxShadow: `0 0 25px -5px ${glowColor}, 0 0 10px -5px ${glowColor}`,
      }}
    >
      <div
...
const Events: React.FC<PageProps> = ({ setActivePage, onBack }) => {
  const [showInfo, setShowInfo] = useState(false);
  const now = new Date().getTime();
  const teamChallengeEndDate =
    now + 3 * 24 * 60 * 60 * 1000 + 5 * 60 * 60 * 1000; // 3 days 5 hours
  const summerFestivalEndDate =
    now + 10 * 24 * 60 * 60 * 1000 + 2 * 60 * 60 * 1000; // 10 days 2 hours
  ...
  const formatCountdown = (time: ReturnType<typeof useCountdown>) => {
    ...
    if (time.days > 0)
      return `${time.days}d ${String(time.hours).padStart(2, "0")}h left`;
    if (time.hours > 0)
      return `${String(time.hours).padStart(2, "0")}h ${String(
        time.minutes
      ).padStart(2, "0")}m left`;
    return `${String(time.minutes).padStart(2, "0")}m ${String(
      time.seconds
    ).padStart(2, "0")}s left`;
  };
      ...
      <AnimatePresence>
        {showInfo && (
          <PageInfoModal title="Events Hub" onClose={() => setShowInfo(false)}>
            <p>
              Welcome to the Events Hub, the vibrant heart of the jungle! This
              is your gateway to dynamic, limited-time challenges offering rare
              rewards and exclusive experiences.
            </p>
            <p className="mt-2">
              From daily games with instant prizes to massive community crusades
              and festive seasonal celebrations, there's always a new adventure
              calling. Check back often—glory awaits!
            </p>
          </PageInfoModal>
        )}
      </AnimatePresence>
